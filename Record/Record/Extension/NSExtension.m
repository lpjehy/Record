...
+ (BOOL)isTodayDate:(NSString *)dateString {
   // NSDate *theDate = [dateString dateWithFormat:@"yyyy-MM-dd"];
    ...
    return NO;
    //BOOL isEarlier = [theDate isEarlierThan:[NSDate date]];
}
...
- (CGSize)sizeByFont:(UIFont *)font
{
    ...
    return [self boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:options attributes:attributes context:nil].size;
...
- (UIImage*)imageWithFilter:(NSString *)filterName {
    //创建CIContext对象(默认值，传入nil)
    CIContext * context = [CIContext contextWithOptions:nil];
    //获取图片
    CIImage * cimage = [CIImage imageWithCGImage:self.CGImage];
    //创建CIFilter
    CIFilter * sepiaTone = [CIFilter filterWithName:filterName];
    //设置滤镜输入参数
    ...
    UIImage * newImg = [[UIImage alloc]initWithCGImage:imageRef];
